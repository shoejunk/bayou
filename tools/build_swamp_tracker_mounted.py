...
def center_body_anchor(img: Image.Image) -> tuple[float, float]:
    box = bounds(img)
    if box is None:
        return CELL / 2, CELL - 18
    l, t, r, b = box
    return (l + r) / 2.0, float(t + (b - t + 1) * 0.82)
...
def build(
    ...
    source = Image.open(SRC / f"SS_swampTracker_mounted_{source_name}.png").convert("RGBA")
    ...
    if mode == "fixed":
        anchors = [center_body_anchor(img) for img in resized]
    ...
    out = OUT / f"swampTracker_mounted-{out_name}.png"
    sheet.save(out)
...
def main() -> None:
    ...
    build("walking", "walk", even_frames(2, 80, 24), 238, 232, "fixed", target_x=128, target_y=220)
    build("attack", "attack", even_frames(2, 68, 12, endpoint=True), 238, 232, "centerbottom", target_x=128, target_y=236)
    build("damaged", "damaged", even_frames(2, 62, 12, endpoint=True), 238, 232, "fixedbottom", target_x=128, target_y=236)
    build("killed", "killed", [2, 7, 12, 17, 22, 27, 31, 35, 38, 41, 45, 50], 252, 236, "fixedbottom", target_x=128, target_y=238)
