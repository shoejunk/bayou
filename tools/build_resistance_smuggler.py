...
TT_TEMP = Path(r"C:\Users\jarox\OneDrive\Desktop\TT Temp")
DOWNLOADS = Path(r"C:\Users\jarox\Downloads")
OUT = ROOT / "assets" / "animations"


def source_path(name: str) -> Path:
    candidate = TT_TEMP / f"SS_resistanceSmuggler_{name}.png"
    if candidate.exists():
        return candidate
    return DOWNLOADS / f"SS_resistanceSmuggler_{name}.png"
...
def build(
    ...
    out = OUT / f"resistanceSmuggler-{out_name}.png"
    sheet.save(out)
...
def main() -> None:
    ...
    build("walking", "walk", even_frames(2, 80, 24), 226, 232, "fixed", target_x=128, target_y=220)
    build("attack", "attack", even_frames(2, 68, 12, endpoint=True), 222, 220, "centerbottom", target_x=128, target_y=236)
    build("damaged", "damaged", even_frames(2, 62, 12, endpoint=True), 230, 226, "fixedbottom", target_x=128, target_y=236)
    build("killed", "killed", even_frames(2, 54, 12, endpoint=True), 252, 236, "center", target_x=128, target_y=238)
