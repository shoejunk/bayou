...
def build(
    ...
    source = Image.open(SRC / f"SS_pipistrelVane_{source_name}.png").convert("RGBA")
    ...
    scale = min(max_w / max_box_w, max_h / max_box_h, 1.12)
    ...
    out = OUT / f"pipistrelVane-{out_name}.png"
    sheet.save(out)
...
def main() -> None:
    ...
    build("walking", "walk", even_frames(6, 75, 24), 158, 236, "planted", 128, 244)
    build("attack", "attack", [8, 12, 16, 20, 24, 29, 35, 41, 47, 53, 60, 66], 226, 240, "center", 128, 244)
    build("damaged", "damaged", [6, 11, 16, 22, 28, 34, 40, 46, 52, 58, 64, 70], 174, 236, "planted", 92, 244)
    build("killed", "killed", [2, 6, 10, 14, 18, 22, 27, 33, 39, 45, 51, 51], 238, 218, "fallen", 18, 240)
