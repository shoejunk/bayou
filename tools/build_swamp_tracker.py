...
def bottom_anchor(img: Image.Image) -> tuple[float, float]:
    ...
    bottom = int(ys.max())
    foot = alpha[max(0, bottom - 8) : bottom + 1] > ALPHA_CUT
...
def body_anchor(img: Image.Image) -> tuple[float, float]:
    box = bounds(img)
    if box is None:
        return CELL / 2, CELL - 18
    l, t, r, b = box
    return (l + r) / 2.0, float(t + (b - t + 1) * 0.84)
...
def build(
    ...
    source = Image.open(SRC / f"SS_swampTracker_{source_name}.png").convert("RGBA")
    ...
    if mode == "feet":
        measured = [bottom_anchor(img) for img in resized]
        stable_x = float(np.median([anchor[0] for anchor in measured]))
        anchors = [(stable_x, anchor[1]) for anchor in measured]
    elif mode == "body":
        measured = [body_anchor(img) for img in resized]
        stable_x = float(np.median([anchor[0] for anchor in measured]))
        anchors = [(stable_x, anchor[1]) for anchor in measured]
    elif mode == "fallen":
        anchors = []
        downed_start = max(4, len(resized) // 2 - 1)
        downed = [left_bottom_anchor(img) for img in resized[downed_start:]]
        stable_left = float(np.median([anchor[0] for anchor in downed]))
        for i, img in enumerate(resized):
            if i >= downed_start:
                _ax, ay = left_bottom_anchor(img)
                anchors.append((stable_left, ay))
            else:
                anchors.append(bottom_anchor(img))
    else:
        anchors = [bottom_anchor(img) for img in resized]
    ...
    out = OUT / f"swampTracker-{out_name}.png"
    sheet.save(out)
...
def main() -> None:
    ...
    build("walking", "walk", even_frames(5, 76, 24), 236, 234, "feet", target_x=124, target_y=239)
    build("attack", "attack", [10, 15, 20, 25, 30, 35, 40, 45, 52, 57, 62, 68], 196, 218, "planted", target_x=72, target_y=239)
    build("damaged", "damaged", even_frames(4, 56, 12), 196, 218, "planted", target_x=74, target_y=239)
    build("killed", "killed", [2, 6, 10, 14, 18, 22, 27, 34, 43, 55, 55, 55], 188, 212, "fallen", target_x=36, target_y=240)
