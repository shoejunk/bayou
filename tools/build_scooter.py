...
def planted_anchor(img: Image.Image) -> tuple[float, float]:
    ...
    if len(xs) == 0:
        return CELL / 2, CELL - 18
    bottom = int(ys.max())
    lower = alpha[max(0, bottom - 10) : bottom + 1] > ALPHA_CUT
...
def build(
    ...
    source = Image.open(SRC / f"SS_scooter_{source_name}.png").convert("RGBA")
    ...
    scale = min(max_w / max_box_w, max_h / max_box_h, 1.85)
    ...
    out = OUT / f"scooter-{out_name}.png"
    sheet.save(out)
...
def main() -> None:
    ...
    build("walking", "walk", even_frames(5, 76, 24), 226, 176, "center", 126, 232)
    build("attack", "attack", [5, 9, 13, 17, 21, 25, 30, 36, 44, 52, 60, 68], 228, 184, "planted", 126, 232)
    build("damaged", "damaged", [4, 8, 12, 16, 21, 27, 33, 39, 46, 53, 61, 69], 224, 190, "planted", 126, 232)
    build("killed", "killed", [2, 6, 10, 14, 18, 22, 27, 32, 37, 42, 42, 42], 232, 204, "fallen", 28, 234)
