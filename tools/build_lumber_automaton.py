...
REVIEW = ROOT / "tools" / "lumberAutomaton-review"

FRAME_SELECTIONS = {
    "walk": [2, 5, 9, 12, 16, 19, 22, 26, 29, 33, 36, 39, 43, 46, 49, 53, 56, 60, 63, 66, 70, 73, 77, 80],
    "attack": [2, 6, 10, 14, 18, 22, 25, 29, 33, 37, 41, 45],
    "damaged": [2, 9, 16, 23, 30, 37, 45, 52, 59, 66, 73, 80],
    "killed": [2, 5, 9, 12, 16, 19, 23, 26, 30, 33, 37, 40],
}
...
def chassis_anchor(image: Image.Image) -> tuple[float, float]:
    ...
    solid = solid.filter(ImageFilter.MinFilter(35))
...
def load_frames() -> dict[str, list[Image.Image]]:
    ...
    for animation, frames in FRAME_SELECTIONS.items():
        ...
        with Image.open(SOURCE / f"SS_lumberAutomaton_{source_name}.png") as opened:
...
def assemble(
    ...
    output = OUTPUT / f"lumberAutomaton-{animation}.png"
...
def main() -> None:
    ...
    assemble("walk", resized["walk"], fixed_chassis_placements(resized["walk"], 128, 140))
    assemble("attack", resized["attack"], lowest_foot_placements(resized["attack"], 108, 244))
    assemble("damaged", resized["damaged"], lowest_foot_placements(resized["damaged"], 128, 244))
    assemble("killed", resized["killed"], lowest_foot_placements(resized["killed"], 128, 244))
...
def write_review_previews() -> None:
    ...
    for animation, frame_numbers in FRAME_SELECTIONS.items():
        with Image.open(OUTPUT / f"lumberAutomaton-{animation}.png") as opened:
            sheet = opened.convert("RGBA")
        frames = [
        ...
        for index, (frame, source_number) in enumerate(zip(frames, frame_numbers)):
            ...
            ImageDraw.Draw(tile).text((5, 5), f"{index + 1}: src {source_number}", fill="white")
            contact.alpha_composite(tile, ((index % columns) * CELL, (index // columns) * CELL))
        contact.convert("RGB").save(REVIEW / f"lumberAutomaton-{animation}-contact.png")
        ...
        gif_frames[0].save(
            REVIEW / f"lumberAutomaton-{animation}.gif",
            save_all=True,
