...
REVIEW = ROOT / "tools" / "maggiesRecruit-review"
SOURCE_REVIEW = ROOT / "tools" / "maggiesRecruit-source-previews"
...
# Each range is evenly sampled over one complete useful motion. Death stops at
# frame 37 before the prone axe arm starts lifting into the recovery sequence.
FRAME_SELECTIONS = {
    "walk": [
...
def body_core_x(image: Image.Image) -> float:
    """Track the torso without allowing the long axe to pull the center."""
    ...
    solid = Image.fromarray(
        np.where((alpha > 32) & (brightness < 200), 255, 0).astype(np.uint8), "L"
    ).filter(ImageFilter.MinFilter(9))
    core = np.asarray(solid) > 0
    yy, _xx = np.indices(core.shape)
    core &= (yy >= top + round(height * 0.12)) & (
...
def ground_contact(image: Image.Image) -> tuple[float, float]:
    """Track the stable viewer-left foot rather than the axe or other boot."""
    array = np.asarray(image.convert("RGBA"))
    alpha = array[..., 3]
    box = bounds(image)
    if box is None:
    ...
    half_width = max(42, round(width * 0.45))
...
def load_raw_frames() -> dict[str, list[Image.Image]]:
    ...
    for animation, frame_numbers in FRAME_SELECTIONS.items():
        source_path = SOURCE / f"SS_maggiesRecruit_{SOURCE_NAMES[animation]}.png"
...
def assemble(
    ...
    path = OUTPUT / f"maggiesRecruit-{animation}.png"
...
def make_source_preview(animation: str) -> Path:
    ...
    path = SOURCE_REVIEW / f"maggiesRecruit-{animation}-source.png"
...
def make_contact_sheet(animation: str, sheet: Image.Image) -> Path:
    ...
    path = REVIEW / f"maggiesRecruit-{animation}-contact.png"
...
def make_edge_preview(animation: str, sheet: Image.Image) -> Path:
    ...
    REVIEW.mkdir(parents=True, exist_ok=True)
    path = REVIEW / f"maggiesRecruit-{animation}-edges.png"
...
def main() -> None:
    ...
    REVIEW.mkdir(parents=True, exist_ok=True)
    report_path = REVIEW / "maggiesRecruit-validation.json"
    report_path.write_text(json.dumps(report, indent=2), encoding="ascii")
    failures = [item for item in report if item["failures"]]
    if failures:
        raise RuntimeError(f"Maggie's Recruit validation failed; see {report_path}")
