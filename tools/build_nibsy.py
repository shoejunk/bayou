...
REVIEW = ROOT / "tools" / "nibsy-review"
SOURCE_REVIEW = ROOT / "tools" / "nibsy-source-previews"
...
# Sample complete motions without the long return-to-idle tails. Death stops
# at frame 45; the source begins raising Nibsy from the floor after frame 54.
FRAME_SELECTIONS = {
    "walk": [
...
def body_core_x(image: Image.Image) -> float:
    """Track Nibsy's torso without the thin staff, lantern, or spell rays."""
    ...
    solid = Image.fromarray(
        np.where((alpha > 32) & (brightness < 180), 255, 0).astype(np.uint8), "L"
...
def ground_contact(image: Image.Image) -> tuple[float, float]:
    """Find a lower foot/body contact and exclude Nibsy's offset equipment."""
    array = np.asarray(image.convert("RGBA"))
    alpha = array[..., 3]
    box = bounds(image)
    if box is None:
    ...
    half_width = max(30, round(width * 0.35))
...
def load_raw_frames() -> dict[str, list[Image.Image]]:
    ...
    for animation, frame_numbers in FRAME_SELECTIONS.items():
        source_path = SOURCE / f"SS_nibsy_{SOURCE_NAMES[animation]}.png"
...
def fixed_attack_placements(
    ...
    local_x = round(128 - (union_left + union_right) / 2.0)
    local_y = round(GROUND_Y - foot_y)
    local_x = min(max(local_x, 2 - union_left), CELL - 3 - union_right)
    local_y = min(max(local_y, 2 - union_top), CELL - 3 - union_bottom)
...
def assemble(
    ...
    path = OUTPUT / f"nibsy-{animation}.png"
...
def make_source_preview(animation: str) -> Path:
    ...
    path = SOURCE_REVIEW / f"nibsy-{animation}-source.png"
...
def make_contact_sheet(animation: str, sheet: Image.Image) -> Path:
    ...
    path = REVIEW / f"nibsy-{animation}-contact.png"
...
def make_edge_preview(animation: str, sheet: Image.Image) -> Path:
    ...
    REVIEW.mkdir(parents=True, exist_ok=True)
    path = REVIEW / f"nibsy-{animation}-edges.png"
