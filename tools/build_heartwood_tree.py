...
def remove_blue_artifacts(img: Image.Image) -> Image.Image:
    arr = zero_transparent_rgb(np.asarray(img.convert("RGBA")))
    r = arr[..., 0].astype(np.int16)
    g = arr[..., 1].astype(np.int16)
    b = arr[..., 2].astype(np.int16)
    a = arr[..., 3] > ALPHA_CUT
    blue = a & (b > 135) & (g > 105) & (r < 190) & ((b - r) > 35) & ((g - r) > 10)
    arr[blue] = 0
    return Image.fromarray(arr, "RGBA")
...
def main() -> None:
    ...
    source = Image.open(SRC / "SS_heartwoodTree_idle.png").convert("RGBA")
    frames = [2, 5, 9, 12, 16, 19, 22, 30, 33, 36, 39, 43, 46, 49, 53, 56, 60, 63, 66, 70, 73, 75, 77, 80]
    raw = [keep_largest_component(remove_blue_artifacts(crop_frame(source, frame))) for frame in frames]
    ...
    scale = min(176 / max_box_w, 236 / max_box_h, 1.0)
    ...
    out = OUT / "heartwoodTree-idle.png"
    sheet.save(out)
