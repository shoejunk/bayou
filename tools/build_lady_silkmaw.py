...
def remove_tiny_components(img: Image.Image, min_pixels: int = 22) -> Image.Image:
...
def planted_anchor(img: Image.Image) -> tuple[float, float]:
    ...
    bottom = int(ys.max())
    lower = alpha[max(0, bottom - 14) : bottom + 1] > ALPHA_CUT
...
def choose_anchors(mode: str, imgs: list[Image.Image]) -> list[tuple[float, float]]:
    ...
    if mode == "planted":
        measured = [planted_anchor(img) for img in imgs]
        stable_x = float(np.median([anchor[0] for anchor in measured]))
        return [(stable_x, anchor[1]) for anchor in measured]
    if mode == "fallen":
        ...
        for i, img in enumerate(imgs):
            if i >= downed_start:
                ...
                anchors.append(planted_anchor(img))
        return anchors
    return [planted_anchor(img) for img in imgs]
...
def build(
    ...
    source = Image.open(SRC / f"SS_ladySilkmaw_{source_name}.png").convert("RGBA")
    ...
    scale = min(max_w / max_box_w, max_h / max_box_h, 1.08)
    ...
    out = OUT / f"ladySilkmaw-{out_name}.png"
    sheet.save(out)
...
def main() -> None:
    ...
    build("walking", "walk", even_frames(5, 76, 24), 238, 226, "planted", 128, 240)
    build("attack", "attack", [5, 9, 13, 17, 21, 25, 29, 33, 37, 41, 45, 50], 226, 222, "planted", 68, 240)
    build("damaged", "damaged", [5, 10, 15, 21, 27, 33, 39, 45, 51, 57, 63, 69], 236, 236, "planted", 128, 242)
    build("killed", "killed", [2, 6, 10, 15, 20, 26, 32, 38, 44, 50, 56, 56], 248, 214, "fallen", 14, 238)
