...
REVIEW = ROOT / "tools" / "theTockman-review"
...
# Attack includes one complete energy flare. Death ends at the last fully
# collapsed source frame before the body begins rising again at frame 51.
FRAME_SELECTIONS = {
    "walk": [
    ...
    "killed": [2, 6, 11, 15, 19, 23, 28, 32, 36, 41, 45, 49],
}
...
def character_mask(image: Image.Image, erode: bool = False) -> np.ndarray:
    """Keep the gold body while excluding the bright neutral energy flare."""
    array = np.asarray(image.convert("RGBA"))
    rgb = array[..., :3]
    alpha = array[..., 3]
    brightness = np.mean(rgb, axis=2)
    chroma = np.max(rgb, axis=2) - np.min(rgb, axis=2)
    mask = (alpha > 28) & ((brightness < 220) | (chroma > 40))
    if erode:
        mask = np.asarray(
...
def body_core_x(image: Image.Image) -> float:
    """Track the cage torso while eroding away hands, toes, wings, and head loop."""
...
def ground_y(image: Image.Image) -> float:
    """Find the lowest central metal toe without following raised outer legs."""
    ...
    half_width = max(42, round(width * 0.43))
    yy, xx = np.indices(mask.shape)
    contact = (
        mask
        & (xx >= center_x - half_width)
        & (xx <= center_x + half_width)
        & (yy >= top + round(height * 0.45))
    )
...
def load_raw_frames() -> dict[str, list[Image.Image]]:
    ...
    for animation, frame_numbers in FRAME_SELECTIONS.items():
        source_path = SOURCE / f"SS_theTockman_{SOURCE_NAMES[animation]}.png"
...
def assemble(
    ...
    path = OUTPUT / f"theTockman-{animation}.png"
...
def make_contact_sheet(animation: str, sheet: Image.Image) -> Path:
    ...
    path = REVIEW / f"theTockman-{animation}-contact.png"
...
def make_edge_preview(animation: str, sheet: Image.Image) -> Path:
    ...
    REVIEW.mkdir(parents=True, exist_ok=True)
    path = REVIEW / f"theTockman-{animation}-edges.png"
