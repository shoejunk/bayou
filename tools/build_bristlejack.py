...
def bbox_bottom_anchor(img: Image.Image) -> tuple[float, float]:
    box = bounds(img)
    if box is None:
        return CELL / 2, CELL - 18
    l, _t, r, b = box
...
def choose_anchors(mode: str, imgs: list[Image.Image]) -> list[tuple[float, float]]:
    if mode == "bottom":
        measured = [bottom_anchor(img) for img in imgs]
    ...
    if mode == "bbox_bottom":
        return [bbox_bottom_anchor(img) for img in imgs]
    if mode == "fallen":
...
def build(
    ...
    source = Image.open(SRC / f"SS_bristlejack_{source_name}.png").convert("RGBA")
    ...
    out = OUT / f"bristlejack-{out_name}.png"
    sheet.save(out)
...
def main() -> None:
    ...
    build("walking", "walk", even_frames(5, 76, 24), 234, 224, "bottom", 126, 238)
    build("attack", "attack", [4, 9, 14, 19, 24, 30, 36, 42, 49, 56, 63, 70], 232, 224, "bottom", 126, 238)
    build("damaged", "damaged", [4, 9, 14, 20, 26, 32, 38, 44, 50, 56, 62, 68], 242, 224, "bottom", 126, 238)
    build("killed", "killed", [2, 7, 12, 18, 24, 30, 36, 42, 48, 54, 54, 54], 236, 210, "bbox_bottom", 128, 238)
