...
def planted_anchor(img: Image.Image) -> tuple[float, float]:
    ...
    bottom = int(ys.max())
    lower = alpha[max(0, bottom - 12) : bottom + 1] > ALPHA_CUT
...
def build(
    ...
    source = Image.open(SRC / f"SS_briarWhisperthorn_{source_name}.png").convert("RGBA")
    ...
    out = OUT / f"briarWhisperthorn-{out_name}.png"
    sheet.save(out)
...
def main() -> None:
    ...
    build("walking", "walk", even_frames(6, 75, 24), 188, 238, "planted", 126, 244)
    build("attack", "attack", [24, 28, 32, 36, 39, 41, 43, 45, 47, 50, 54, 58], 226, 240, "center", 128, 244)
    build("damaged", "damaged", [6, 11, 16, 22, 28, 34, 40, 46, 52, 58, 64, 70], 188, 238, "planted", 126, 244)
    build("killed", "killed", [2, 7, 12, 17, 22, 27, 32, 36, 39, 39, 39, 39], 242, 238, "fallen", 22, 244)
