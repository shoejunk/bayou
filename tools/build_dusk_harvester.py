...
def lower_anchor(img: Image.Image) -> tuple[float, float]:
    box = bounds(img)
    if box is None:
    ...
    region = (alpha > 80) & (yy >= max(0, b - 24)) & (yy <= b) & (xx >= l + (r - l + 1) * 0.18) & (xx <= l + (r - l + 1) * 0.82)
    ys, xs = np.nonzero(region)
    if len(ys) == 0:
        return (l + r) / 2.0, float(b)
    y = int(np.percentile(ys, 99))
    band = region & (yy >= y - 10) & (yy <= y)
...
def build(name: str, frames: list[int], max_w: int, max_h: int, mode: str, target_x: int = 128, target_y: int = 238) -> None:
    source = Image.open(SRC / f"SS_duskHarvester_{name}.png").convert("RGBA")
    ...
    scale = min(max_w / max_box_w, max_h / max_box_h, 1.14)
    ...
    if mode == "fixedcrop":
        ...
        dx0 = round((CELL - resized[0].width) / 2) + target_x
        ...
        out = OUT / f"duskHarvester-walk.png"
        sheet.save(out)
        print(out.name, sheet.size, "frames=" + ",".join(str(frame) for frame in frames))
        return
    anchors = [center_bottom_anchor(img) if mode == "center" else lower_anchor(img) for img in resized]
    stable_x = float(np.median([anchor[0] for anchor in anchors]))
    sheet = Image.new("RGBA", (CELL * len(frames), CELL), (0, 0, 0, 0))
    for i, img in enumerate(resized):
        ax, ay = anchors[i]
        dx = round(i * CELL + target_x - (ax if mode == "footlock" else stable_x))
    ...
    out = OUT / f"duskHarvester-{out_name}.png"
    sheet.save(out)
    print(out.name, sheet.size, "frames=" + ",".join(str(frame) for frame in frames))
...
def main() -> None:
    ...
    build("walking", [2, 5, 9, 11, 14, 17, 21, 24, 27, 29, 31, 35, 40, 44, 47, 51, 57, 59, 61, 64, 66, 70, 73, 76], 224, 228, "fixedcrop", target_x=-16, target_y=8)
    build("attack", [2, 8, 14, 20, 26, 32, 38, 44, 50, 56, 62, 68], 222, 228, "footlock", target_x=102)
    build("damaged", [2, 8, 14, 20, 26, 32, 38, 44, 50, 56, 62, 68], 218, 226, "footlock", target_x=102)
