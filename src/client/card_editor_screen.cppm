...
private:
    ...
    void drawInstructions(sf::RenderWindow& window)
    {
        ...
        y = drawInstructionBullet(window, "ability: transform, dematerialize, dig, summon, command, or trail. Transform-style abilities switch action states; dig creates a tunnel hole; summon creates the unit named by the summon string field in the space in front; command lets one ready adjacent friendly piece take any normal action without ending the turn; trail passively creates the named unit where this piece started whenever it moves.", y);
        y = drawInstructionBullet(window, "summon: exact Unit card title created by a summon or trail ability. Player 1 summons to the right and Player 2 to the left; trail summons into the moving piece's former position. Summoned units cannot act until their owner's next turn.", y);
