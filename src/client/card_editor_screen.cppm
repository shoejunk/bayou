...
private:
    ...
    void drawInstructions(sf::RenderWindow& window)
    {
        ...
        y = drawInstructionBullet(window, "Keywords are stored separately from traits. Relentless lets a piece act again immediately whenever it destroys another piece. Bodyguard passively redirects damage from adjacent friendly non-Bodyguard pieces to itself; multiple Bodyguards split that damage as evenly as possible. Intercept lets an unused adjacent friendly Intercept piece swap places with an attacked piece and take the attack; it can be used once per turn, and Bodyguard takes priority. Trail passively creates the Unit named by the summon string field where this piece started whenever it moves. Foresight lets a player reveal one more card than the number of controlled Foresight Units at the start of their turn, keep one, and put the rest on the bottom of the deck.", y);
