    auto drawStoryIntro = [&]() {
        ...
        // Establish the scene and its goal once, then let the characters
        // speak on the board. The last page repeats a short goal before Start.
        const bool showMissionGoal = !storyOnly;
        if (storyComicPage == 0)
        {
            storySpeechBubbleBounds = {};
        ...
        {
            prepareStoryScenePreview(mission);
            drawGame(&storyScenePreview, !storyOnly);
            drawTitlePlaque(window, font, std::string(mission.title),
                {400.0f, 31.0f}, {704.0f, 46.0f});
            drawStorySpeechBubble(panel, storyScenePreview.pieces, 1);
            if (!storyOnly && storyComicPage + 1 >= panelCount)
            {
                const float goalBottom = drawWrappedText(window, font,
                    "Goal: " + std::string(mission.objective), 13,
                    {48.0f, 466.0f}, BoardParchment, 704.0f, 2.0f);
                if (captureRequest && goalBottom > 516.0f)
                    failCaptureValidation("Dialogue goal summary exceeds its footer.");
            }
        }
        ...
        drawCenteredText(
            window,
            font,
            (storyComicPage == 0 ? "SETUP " : "DIALOGUE ") + std::to_string(storyComicPage + 1) + " OF " +
